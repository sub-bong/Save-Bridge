...
- 일반 문장은 자연스러운 한국어로 표현하되, 증상·상태·생체징후·진단명·의식 수준 등 **의학 관련 표현(약어 포함)은 응급의료 문맥에 맞는 표준 영어 의학 용어로 정규화**해야 한다.
...
- 이 텍스트를 분석하여, **전반적인 문장은 자연스러운 한국어로**, 

- 그 안에 포함된 **의학 관련 표현(증상, 상태, 생체징후, 의식 수준, 진단명, 약어 등)은 표준 영어 의학 용어**로 정규화하여 표기하세요.

- 예: "숨이 안 쉬어짐" → 상황에 따라 "severe dyspnea" 또는 "respiratory arrest",  

      "말이 꼬인다" → "dysarthria",  

      "말이 잘 안 나온다" → "aphasia" 등.

출력 형식:

- 내가 전달한 모든 문장을 **하나도 빠뜨리지 말고** 번역하세요. (요약·축약 금지)
...
1. 일반적인 설명/문장은 자연스러운 한국어 문장으로 작성합니다.

2. 의학 관련 용어(예: V/S, TPR, HR, BP, RR, SpO2, mental state, GCS 등)와 질환/상태/검사명은 응급의료 문맥에 맞는 **표준 영어 의학 용어**로 변환하여 표기합니다.

3. 구급대원이 사용하는 비표준 표현은 문맥을 보고 가능한 한 표준 용어로 변환합니다.
...
4. 오타·발음 착오 등으로 애매한 표현은, 가능한 경우 가장 안전하고 넓은 범위의 의학 표현으로 변환합니다.
   ...
   - 예: "20분 전 추정" → "about 20 minutes ago"

5. 명확하지 않은 진단을 확정적으로 쓰지 말고, "is suspected", "likely", "suggestive of" 등을 사용해 **의심 수준**으로 표현합니다.
...
2. 의학 관련 용어는 응급의료 문맥에 맞는 **표준 영어 의학 용어**로 변환하여 표기합니다.

3. Pre-KTAS 점수 또는 등급 정보는 반드시 정확히 보존하세요.
   - "Pre-KTAS 1점" ~ "Pre-KTAS 5점" 형식으로 표기
   - 다양한 변형 표현을 "Pre-KTAS"로 통일
   - 점수 숫자(1-5)는 반드시 정확히 보존
