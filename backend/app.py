...
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False  # 비활성화 권장
...
"""
        ...
        # GPT-4-turbo로 SBAR 형식 변환
        sbar_prompt = f"""아래는 응급의료 현장에서 구급대원이 보고한 환자 상태 정보입니다.
이 정보를 SBAR (Situation, Background, Assessment, Recommendation) 형식으로 구조화하여 변환하세요.

SBAR 형식:
- S (Situation): 현재 상황 - 환자 기본 정보(나이, 성별), 주요 증상, 증상 발생 시점
- B (Background): 배경 정보 - 기저질환, 생체 징후(V/S), 과거력
- A (Assessment): 평가 - 의식 상태, 중증도 평가(Pre-KTAS), 주요 의심 질환
- R (Recommendation): 권고 - 필요한 이송 병원 유형, 긴급도

규칙:
1. 각 섹션은 명확하게 구분하여 작성
2. 의학 용어는 영어로 표기 (예: BP, HR, GCS, FAST 등)
3. Pre-KTAS 레벨이 명시되어 있으면 반드시 포함
4. 시간 정보가 있으면 반드시 포함 (예: "발생 20분 전")
5. 생체 징후 수치가 있으면 반드시 포함 (예: "BP 180/100", "HR 120", "SpO2 95%")
6. 간결하고 명확하게 작성
...
SBAR 형식으로 변환:"""
        ...
        sbar_completion = openai_client.chat.completions.create(
            model="gpt-4-turbo",
            messages=[
                {"role": "system", "content": "너는 응급의료 현장의 환자 정보를 SBAR 형식으로 구조화하는 전문가이다. 구급대원의 보고 내용을 체계적으로 정리하여 병원 전달에 최적화된 형식으로 변환한다."},
                {"role": "user", "content": sbar_prompt}
            ],
            temperature=0.3
        )
        ...
        # SBAR 형식을 자연스러운 문장으로 변환 (ARS 서비스용)
        print("[STT] SBAR → 자연스러운 문장 변환 시작...")
        try:
            ars_narrative_prompt = f"""아래는 SBAR 형식으로 구조화된 응급환자 정보입니다.
이 정보를 ARS(자동응답서비스)에서 의료진이 쉽게 이해할 수 있도록 자연스러운 하나의 문장으로 변환하세요.

요구사항:
1. SBAR의 S(상황), B(배경), A(평가), R(권고) 순서대로 자연스럽게 연결
2. 의학 용어는 그대로 유지 (예: BP, HR, GCS, FAST 등)
3. 숫자와 수치는 명확하게 전달
4. 시간 정보가 있으면 자연스럽게 포함
5. 하나의 완전한 문장으로 구성 (너무 길면 2-3문장으로 나눌 수 있음)
6. 구어체가 아닌 정중하고 전문적인 문체 사용
7. "입니다", "합니다" 등 존댓말 사용
...
자연스러운 문장으로 변환:"""
            ...
            ars_narrative_completion = openai_client.chat.completions.create(
                model="gpt-4-turbo",
                messages=[
                    {"role": "system", "content": "너는 응급의료 현장의 SBAR 형식 정보를 의료진이 쉽게 이해할 수 있는 자연스러운 문장으로 변환하는 전문가이다. ARS 서비스를 통해 전달될 때 의료진이 한 번에 이해할 수 있도록 명확하고 자연스럽게 변환한다."},
                    {"role": "user", "content": ars_narrative_prompt}
                ],
                temperature=0.3
            )
...
# 병원에 전화 걸기 및 RequestAssignment 생성 API
